...
printf "
#######################################################################
#            For CentOS/RedHat 6+ Debian 8+ and Ubuntu 14+            #
#######################################################################
"
# Check if user is root
[ $(id -u) != "0" ] && { echo "${CFAILURE}Error: You must be root to run this script${CEND}"; exit 1; }

allinstall_dir=$(dirname "`readlink -f $0`")
pushd ${allinstall_dir} > /dev/null
. ./versions.txt
. ./options.conf
. ./include/color.sh
. ./include/check_os.sh
...
checkDownload 2>&1 | tee -a ${allinstall_dir}/install.log
...
if [ ! -e ~/.allinstall ]; then
  ...
  case "${OS}" in
    "CentOS")
      installDepsCentOS 2>&1 | tee ${allinstall_dir}/install.log
      . include/init_CentOS.sh 2>&1 | tee -a ${allinstall_dir}/install.log
      ;;
    "Debian")
      installDepsDebian 2>&1 | tee ${allinstall_dir}/install.log
      . include/init_Debian.sh 2>&1 | tee -a ${allinstall_dir}/install.log
      ;;
    "Ubuntu")
      installDepsUbuntu 2>&1 | tee ${allinstall_dir}/install.log
      . include/init_Ubuntu.sh 2>&1 | tee -a ${allinstall_dir}/install.log
      ;;
  esac
  # Install dependencies from source package
  installDepsBySrc 2>&1 | tee -a ${allinstall_dir}/install.log
fi
...
if [[ ${nginx_option} =~ ^[1-3]$ ]] || [[ "${db_option}" =~ ^[1-9]$|^1[0-3]$ ]]; then
  . include/jemalloc.sh
  Install_Jemalloc | tee -a ${allinstall_dir}/install.log
fi

# openSSL
if [[ ${tomcat_option} =~ ^[1-4]$ ]] || [[ ${apache_option} =~ ^[1-2]$ ]] || [[ ${php_option} =~ ^[1-9]$|^10$ ]] || [[ "${mphp_ver}" =~ ^5[3-6]$|^7[0-4]$|^80$ ]]; then
  . include/openssl.sh
  Install_openSSL | tee -a ${allinstall_dir}/install.log
fi
...
case "${db_option}" in
  1)
    ...
    Install_MySQL80 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  2)
    . include/mysql-5.7.sh
    Install_MySQL57 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  3)
    . include/mysql-5.6.sh
    Install_MySQL56 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  4)
    . include/mysql-5.5.sh
    Install_MySQL55 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  5)
    . include/mariadb-10.5.sh
    Install_MariaDB105 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  6)
    . include/mariadb-10.4.sh
    Install_MariaDB104 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  7)
    . include/mariadb-10.3.sh
    Install_MariaDB103 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  8)
    . include/mariadb-5.5.sh
    Install_MariaDB55 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  9)
    [ "${OS}" == 'CentOS' ] && [ ${CentOS_ver} -le 6 >/dev/null 2>&1 ] && dbinstallmethod=1 && checkDownload
    [ "${OS}" == 'CentOS' ] && [ "${CentOS_ver}" == '8' ] && dbinstallmethod=2 && checkDownload
    . include/percona-8.0.sh
    Install_Percona80 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  10)
    . include/percona-5.7.sh
    Install_Percona57 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  11)
    . include/percona-5.6.sh
    Install_Percona56 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  12)
    . include/percona-5.5.sh
    Install_Percona55 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  13)
    . include/alisql-5.6.sh
    Install_AliSQL56 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  14)
    . include/postgresql.sh
    Install_PostgreSQL 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  15)
    . include/mongodb.sh
    Install_MongoDB 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
esac
...
case "${nginx_option}" in
  1)
    . include/nginx.sh
    Install_Nginx 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  2)
    . include/tengine.sh
    Install_Tengine 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  3)
    . include/openresty.sh
    Install_OpenResty 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
esac
...
if [ "${apache_option}" == '1' ]; then
  ...
  Install_Apache24 2>&1 | tee -a ${allinstall_dir}/install.log
elif [ "${apache_option}" == '2' ]; then
  . include/apache-2.2.sh
  Install_Apache22 2>&1 | tee -a ${allinstall_dir}/install.log
fi
...
case "${php_option}" in
  1)
    . include/php-5.3.sh
    Install_PHP53 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  2)
    . include/php-5.4.sh
    Install_PHP54 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  3)
    . include/php-5.5.sh
    Install_PHP55 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  4)
    . include/php-5.6.sh
    Install_PHP56 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  5)
    . include/php-7.0.sh
    Install_PHP70 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  6)
    . include/php-7.1.sh
    Install_PHP71 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  7)
    . include/php-7.2.sh
    Install_PHP72 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  8)
    . include/php-7.3.sh
    Install_PHP73 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  9)
    . include/php-7.4.sh
    Install_PHP74 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  10)
    . include/php-8.0.sh
    Install_PHP80 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
esac
...
PHP_addons() {
  ...
  case "${phpcache_option}" in
    1)
      ...
      Install_ZendOPcache 2>&1 | tee -a ${allinstall_dir}/install.log
      ;;
    2)
      . include/xcache.sh
      Install_XCache 2>&1 | tee -a ${allinstall_dir}/install.log
      ;;
    3)
      . include/apcu.sh
      Install_APCU 2>&1 | tee -a ${allinstall_dir}/install.log
      ;;
    4)
      . include/eaccelerator.sh
      Install_eAccelerator 2>&1 | tee -a ${allinstall_dir}/install.log
      ;;
  esac
  ...
  if [ "${pecl_zendguardloader}" == '1' ]; then
    ...
    Install_ZendGuardLoader 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # ioncube
  if [ "${pecl_ioncube}" == '1' ]; then
    . include/ioncube.sh
    Install_ionCube 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # SourceGuardian
  if [ "${pecl_sourceguardian}" == '1' ]; then
    . include/sourceguardian.sh
    Install_SourceGuardian 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # imagick
  if [ "${pecl_imagick}" == '1' ]; then
    . include/ImageMagick.sh
    Install_ImageMagick 2>&1 | tee -a ${allinstall_dir}/install.log
    Install_pecl_imagick 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # gmagick
  if [ "${pecl_gmagick}" == '1' ]; then
    . include/GraphicsMagick.sh
    Install_GraphicsMagick 2>&1 | tee -a ${allinstall_dir}/install.log
    Install_pecl_gmagick 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # fileinfo
  if [ "${pecl_fileinfo}" == '1' ]; then
    . include/pecl_fileinfo.sh
    Install_pecl_fileinfo 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # imap
  if [ "${pecl_imap}" == '1' ]; then
    . include/pecl_imap.sh
    Install_pecl_imap 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # ldap
  if [ "${pecl_ldap}" == '1' ]; then
    . include/pecl_ldap.sh
    Install_pecl_ldap 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # calendar
  if [ "${pecl_calendar}" == '1' ]; then
    . include/pecl_calendar.sh
    Install_pecl_calendar 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # phalcon
  if [ "${pecl_phalcon}" == '1' ]; then
    . include/pecl_phalcon.sh
    Install_pecl_phalcon 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # yaf
  if [ "${pecl_yaf}" == '1' ]; then
    . include/pecl_yaf.sh
    Install_pecl_yaf 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # yar
  if [ "${pecl_yar}" == '1' ]; then
    . include/pecl_yar.sh
    Install_pecl_yar 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # pecl_memcached
  if [ "${pecl_memcached}" == '1' ]; then
    . include/memcached.sh
    Install_pecl_memcached 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # pecl_memcache
  if [ "${pecl_memcache}" == '1' ]; then
    . include/memcached.sh
    Install_pecl_memcache 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # pecl_redis
  if [ "${pecl_redis}" == '1' ]; then
    . include/redis.sh
    Install_pecl_redis 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # pecl_mongodb
  if [ "${pecl_mongodb}" == '1' ]; then
    . include/pecl_mongodb.sh
    Install_pecl_mongodb 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # swoole
  if [ "${pecl_swoole}" == '1' ]; then
    . include/pecl_swoole.sh
    Install_pecl_swoole 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # xdebug
  if [ "${pecl_xdebug}" == '1' ]; then
    . include/pecl_xdebug.sh
    Install_pecl_xdebug 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # pecl_pgsql
  if [ -e "${pgsql_install_dir}/bin/psql" ]; then
    . include/pecl_pgsql.sh
    Install_pecl_pgsql 2>&1 | tee -a ${allinstall_dir}/install.log
  fi
}
...
if [ "${mphp_flag}" == 'y' ]; then
  . include/mphp.sh
  Install_MPHP 2>&1 | tee -a ${allinstall_dir}/install.log
...
case "${jdk_option}" in
  1)
    . include/jdk-11.0.sh
    Install_JDK110 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  2)
    . include/jdk-1.8.sh
    Install_JDK18 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  3)
    . include/jdk-1.7.sh
    Install_JDK17 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  4)
    . include/jdk-1.6.sh
    Install_JDK16 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
esac
...
case "${tomcat_option}" in
  1)
    . include/tomcat-9.sh
    Install_Tomcat9 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  2)
    . include/tomcat-8.sh
    Install_Tomcat8 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  3)
    . include/tomcat-7.sh
    Install_Tomcat7 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
  4)
    . include/tomcat-6.sh
    Install_Tomcat6 2>&1 | tee -a ${allinstall_dir}/install.log
    ;;
esac
...
if [ "${pureftpd_flag}" == 'y' ]; then
  . include/pureftpd.sh
  Install_PureFTPd 2>&1 | tee -a ${allinstall_dir}/install.log
fi

# phpMyAdmin
if [ "${phpmyadmin_flag}" == 'y' ]; then
  . include/phpmyadmin.sh
  Install_phpMyAdmin 2>&1 | tee -a ${allinstall_dir}/install.log
fi

# redis
if [ "${redis_flag}" == 'y' ]; then
  . include/redis.sh
  Install_redis_server 2>&1 | tee -a ${allinstall_dir}/install.log
fi

# memcached
if [ "${memcached_flag}" == 'y' ]; then
  . include/memcached.sh
  Install_memcached_server 2>&1 | tee -a ${allinstall_dir}/install.log
fi

# index example
if [ -d "${wwwroot_dir}/default" ]; then
  . include/demo.sh
  DEMO 2>&1 | tee -a ${allinstall_dir}/install.log
fi
...
if [ "${hhvm_flag}" == 'y' ] && [ "${PM}" == 'yum' -a "${OS_BIT}" == '64' ] && [ -n "`grep -E ' 7\.| 6\.[5-9]' /etc/redhat-release`" ]; then
  ...
  Install_hhvm_CentOS 2>&1 | tee -a ${allinstall_dir}/install.log
fi

# Python
if [ "${python_flag}" == 'y' ]; then
  . include/python.sh
  Install_Python 2>&1 | tee -a ${allinstall_dir}/install.log
fi
...
echo "Total allinstall Install Time: ${CQUESTION}${installTime}${CEND} minutes"
