...
printf "
#######################################################################
#       AllInstall for CentOS/RedHat 6+ Debian 8+ and Ubuntu 14+      #
#              Upgrade Software versions for AllInstall               #
#######################################################################
"
...
Show_Help() {
  ...
  --allinstall                Upgrade allinstall latest
  --acme.sh                   Upgrade acme.sh latest
  "
}

ARG_NUM=$#
TEMP=`getopt -o h --long help,nginx:,tengine:,openresty:,apache:,tomcat:,db:,php:,redis:,memcached:,phpmyadmin:,allinstall,acme.sh -- "$@" 2>/dev/null`
...
while :; do
  ...
  case "$1" in
    ...
    --phpmyadmin)
      ...
      ;;
    --allinstall)
      NEW_allinstall_ver=latest; shift 1
      ;;
    --acme.sh)
...
\t${CMSG} 9${CEND}. Upgrade allinstall latest
...
if [ ${ARG_NUM} == 0 ]; then
  ...
  [ "${NEW_allinstall_ver}" == 'latest' ] && Upgrade_allinstall
