...
Show_Help() {
  ...
  --help, -h                    Show this help message, More: https://google.com
...
Print_Warn() {
  echo
  echo "${CWARNING}You will uninstall allinstall, Please backup your configure files and DB data! ${CEND}"
}
...
Uninstall_PHPext() {
  ...
  if [ "${pecl_yaf}" == '1' ]; then
    . include/pecl_yaf.sh
    Uninstall_pecl_yaf 2>&1 | tee -a ${allinstall_dir}/install.log
  fi

  # yar
  if [ "${pecl_yar}" == '1' ]; then
    . include/pecl_yar.sh
    Uninstall_pecl_yar 2>&1 | tee -a ${allinstall_dir}/install.log
  fi
