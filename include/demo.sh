...
DEMO() {
  ...
  if [ ! -e ${wwwroot_dir}/default/index.html ]; then 
    [ "${IPADDR_COUNTRY}"x == "IN"x ] && /bin/cp ${allinstall_dir}/config/index_in.html ${wwwroot_dir}/default/index.html || /bin/cp ${allinstall_dir}/config/index.html ${wwwroot_dir}/default
  fi
