...
Install_Panel() {
  ...
  sed -i "s@/root/allinstall/options.conf@${allinstall_dir}/options.conf@" panel/settings.py
  ...
  if [ -e /bin/systemctl ]; then
    /bin/cp ${allinstall_dir}/init.d/panel.service /lib/systemd/system/
    sed -i "s@/root/git/repo/panel@`pwd`@g" /lib/systemd/system/panel.service
    systemctl enable panel
  else
    /bin/cp ${allinstall_dir}/init.d/Panel-init /etc/init.d/panel
...
Upgrade_Panel() {
  pushd ${allinstall_dir}/../panel > /dev/null
  git pull
